...
int main() {
    ...
    string table[6][10] =
    {
        ...
        //  a       +       -       *       /        (       )    =   $     b
          {"F=E" ,  "0",   "0",   "0",   "0",    "F=E",   "0",   "0", "0", "F=E"}, //  S
          {"TQ" ,  "0",    "0",   "0",   "0",     "TQ",   "0",   "0", "0", "TQ"},  // E
          {"0" ,  "+TQ",   "-TQ",  "0", "0",     "0",    "z",   "0", "z",  "0"}, // Q
          {"FR" ,  "0",    "0",    "0",  "0",     "FR",   "0",   "0", "0",  "FR"}, // T
          {"0" ,   "z",   "z",   "*FR",   "/FR",   "0",   "z",   "0",  "z",  "0"}, // R
          {"a" ,   "0",   "0",   "0",     "0",    "(E)",  "0",   "0",  "0",  "b"} //F
    ...
    while (true)
    {
        ...
        stacky.push("$");
        stacky.push("S"); // starting state is S
        int charOfString;
        ...
        {
            if (stacky.size() == 1 && tempInput.size() > 1)
            {
                cout << "ERROR cant have a following character after terminating symbol \"$\" " << endl;
                break;
            }
...
{ // checks whether stringx is a terminal state return false if its is not a terminal
    ...
    string acceptedString[12] = { "a", "+", "-","*","/","(",")","=", "b", "$","z", "0" };// these are terminals
